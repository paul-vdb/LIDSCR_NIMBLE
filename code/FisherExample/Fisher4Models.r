...
# plot(out[,c('sigma', 'lambda', 'N', 'D')])
# valueInCompiledNimbleFunction(Cmcmc$samplerFunctions[[1]], "scale")
...
plot(density(out2D), main = "Animal Density", ylim = c(0,1.5))
lines(density(out1D), col = "blue")
lines(density(out[,'D']), col = 'red')

####################################
# Model 3: Sex and collar as covariates
####################################
Model4 <- nimbleCode({
    lambda ~ dunif(0, 20) # Detection rate at distance 0
    psi ~ dbeta(1, 1)      # prior on data augmentation bernoulli vec.
